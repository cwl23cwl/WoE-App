...
export default function WorkspaceEditorPage() {
  ...
  const handleCanvasChange = useCallback(() => {
    if (isCanvasReady && excalidrawAPIRef.current) {
      // Mark as unsaved immediately when changes are made
      const { setSaveState, updateCurrentPage } = useWorkspaceStore.getState()
  ...
  // Set up auto-save interval and change listener
  useEffect(() => {
    if (!isCanvasReady || !excalidrawAPIRef.current) return
    ...
    const checkForChanges = () => {
      try {
        ...
        if (elements && appState) {
          ...
          const currentTimestamp = appState.timestamp || 0
          
          // Check if there are actual changes
          if (currentElementsCount !== lastElementsCount || currentTimestamp !== lastTimestamp) {
            lastElementsCount = currentElementsCount
