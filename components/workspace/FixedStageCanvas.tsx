...
interface FixedStageCanvasProps {
  className?: string
}

export function FixedStageCanvas({ className = '' }: FixedStageCanvasProps) {
  ...
  }, [activeTool, showMarginGuides, pages, currentPageIndex, currentPage])

  // Load Excalidraw dynamically
  useEffect(() => {
    console.log('🔄 Starting Excalidraw load for FixedStageCanvas')
    const loadExcalidraw = async () => {
      try {
        console.log('🔄 Importing @excalidraw/excalidraw...')
        const { Excalidraw } = await import('@excalidraw/excalidraw')
        console.log('✅ Excalidraw import successful:', Excalidraw)
        ExcalidrawComponent = Excalidraw
        setIsLoaded(true)
        console.log('✅ Fixed Stage Excalidraw loaded successfully')
      } catch (err) {
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          borderRadius: '8px',
          boxShadow: '0 4px 12px rgba(0, 0, 0, 0.08)', // Professional shadow
          position: 'relative',
        ...
        <div style={{ width: '100%', height: '100%', position: 'relative' }}>
          <ExcalidrawComponent
            ...
            UIOptions={{
              ...
              // Hide all zoom/pan related UI
              tools: {
                image: false, // Often causes zoom issues
              }
            }}
            // Disable trackpad/mouse zoom
            onPointerDown={(e) => {
              // Block zoom gestures
