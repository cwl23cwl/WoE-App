...
export function TopToolbar({ onUndo, onRedo }: TopToolbarProps) {
  ...
  // Keep the secondary toolbar open on same-tool clicks.
  const [expandedTool, setExpandedTool] = useState<UIToolId | null>(null);

  // naive availability (enable buttons when API exists)
  ...
  const handleToolSelect = useCallback(
    (tool: Tool) => {
      try {
        ...
        // NOTE: store ToolType may not include 'select' or 'erase' yet — see note below.
        // We cast to any to avoid typing errors until you extend ToolType.
  ...
  // map SaveState ('idle' | 'saving' | 'saved' | 'error') to UI
  const saveBadge = useMemo(() => {
    ...
    // 'idle' or anything else → neutral/ready
