...
export default function StudentWorkspace({ submissionId = 'demo-submission' }: StudentWorkspaceProps) {
  ...
  const handleFontSizeChange = useCallback((newSize: number) => {
    ...
    // Also update overlay immediately for live feedback during editing
    const editingElementId = workspaceEditingTextId || editingTextId;
    if (editingElementId) {
      ...
      if (overlayElement) {
        ...
        console.log('🎨 Updated overlay font size immediately');
      }
    }
  }, [excalidrawAPI, isMounted, editingTextId, workspaceEditingTextId, applyTextStyleToSelection]);
  ...
  const handleChange = useCallback((elements: any, appState: any) => {
    ...
    const previousSelectedIdsSet = new Set(selectedElementIds);
    
    // Update selection if it changed
    const selectionChanged = 
      currentSelectedIds.length !== selectedElementIds.length ||
      currentSelectedIds.some((id: string) => !previousSelectedIdsSet.has(id)) ||
      selectedElementIds.some(id => !currentSelectedIdsSet.has(id));
  ...
  }, [pages, currentPageIndex, submissionId, setSaveState, zoomLevel, workspaceEditingTextId, setWorkspaceEditingTextId, isEditing, selectedElementIds, setSelectedElementIds]); // Added dependencies
    ...
    try {
      ...
      excalidrawAPI.updateScene({ 
        ...
        appState: { 
          ...
          currentItemRoughness: 1,
          currentItemOpacity: 100,
          viewModeEnabled: role === 'TEACHER' ? true : false,
          zenModeEnabled: true,
          ...scene.appState
        } 
      });
  ...
  }, [excalidrawAPI, isMounted, isLoadingPages, pages, currentPageIndex, role, setSaveState]); // Removed dynamic color/width dependencies
      ...
      <div className="p-4 bg-white border-b border-border shadow-soft flex-shrink-0" data-toolbar="true">
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <div className="flex gap-2 items-center border-l border-border pl-4">
            ...
            {activeTool === 'text' && (
              <ColorPopover
                currentColor={derivedStyle.color}
          ...
          {activeTool === 'text' && (
            <div className="border-l border-border pl-4">
              <TextStyleControls
                ...
                fontSize={derivedStyle.fontSize}
                onFontSizeChange={handleFontSizeChange}
                isBold={derivedStyle.bold}
                isItalic={derivedStyle.italic}
                isUnderlined={derivedStyle.underline}
                isMixed={isMixed}
      ...
      <div className="flex-1 bg-background">
          ...
          <div style={{ height: '100%', width: '100%' }}>
            <Excalidraw
              ...
              onPointerDown={(e) => {
                // Block canvas interactions during text editing
                if (workspaceEditingTextId || editingTextId) {
                  console.log('🎯 Blocking canvas pointer down - text editing active');
                  e.preventDefault();
                  e.stopPropagation();
                  return false;
                }
              ...
              onPointerMove={(e) => {
                // Block canvas pointer move during text editing (prevents pan/draw)
                if (workspaceEditingTextId || editingTextId) {
                  console.log('🎯 Blocking canvas pointer move - text editing active');
                  e.preventDefault();
                  e.stopPropagation();
                  return false;
                }
              ...
              onPointerUp={(e) => {
                // Block canvas pointer up during text editing
                if (workspaceEditingTextId || editingTextId) {
                  console.log('🎯 Blocking canvas pointer up - text editing active');
                  e.preventDefault();
                  e.stopPropagation();
                  return false;
                }
              ...
              onWheel={(e) => {
                // Allow zoom during text editing but prevent other wheel interactions
                if (workspaceEditingTextId || editingTextId) {
                  // Only allow zoom with Ctrl/Cmd key
                  if (!e.ctrlKey && !e.metaKey) {
                    console.log('🎯 Blocking wheel event - text editing active (no zoom modifier)');
                    e.preventDefault();
                    e.stopPropagation();
                    return false;
                  }
